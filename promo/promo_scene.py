"""eNI — production promo video with synced narration."""
from manim import *
import json
import os
...
class ProductPromo(Scene):
    def construct(self):
        ...
        # ═══ INTRO ═══
        title = Text("eNI", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("Embedded Neural Interface Framework", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        # Tech badges
        techs = "C, DSP, ARM CMSIS".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # ═══ FEATURES ═══
        features = [
            ("01", "Cross-Platform Neural I/O", "Unified API connects to EEG, EMG, and BCI hardware from any vendor", DUR["f1"]),
            ("02", "Real-Time Signal Processing", "DSP pipeline processes neural data at millisecond precision with configurable filters", DUR["f2"]),
            ("03", "Hardware Abstraction Layer", "Swap sensors without changing application code using pluggable driver modules", DUR["f3"]),
        ]
        ...
        components = ["Sensor HAL", "DSP Pipeline", "Feature Extract", "Classifier", "Output"]
        boxes = VGroup()
